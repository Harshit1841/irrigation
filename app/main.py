...
async def lifespan(app: FastAPI):
    ...
    async with AsyncSessionLocal() as db:
        ...
        count = result.scalar()
        if count == 0:
            print("Plant table empty - seeding default plants...")
            from seed import seed as seed_default

            await seed_default()
            print("Seeding default plants complete!")

            print("Seeding Indian plants...")
            from seed_indian_plants import seed_database_sync

            await seed_database_sync()
            print("Seeding Indian plants complete!")
